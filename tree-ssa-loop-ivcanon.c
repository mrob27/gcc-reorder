...
/* %%% RML1   dbf1.h: To write to debug log file */
#include "dbf1.h"
...
pass_complete_unrolli::execute (function *fun)
{
  ...
  nl = (long) (number_of_loops (fun));
fprintf(rml_dbf1, "cunroll: %ld loops in function\n", nl);
  if (nl > 1)
    {
      scev_initialize ();
fprintf(rml_dbf1, "cunroll: running for opt level %d\n", ((int)optimize));
      ret = tree_unroll_loops_completely (optimize >= 3, false);
      scev_finalize ();
    }
  loop_optimizer_finalize ();

fprintf(rml_dbf1, "cunroll: returning %d\n", ((int) ret));
  return ret;
}
