...
__attribute__((const))
double mg2(const double *A, int n)
{
  double sum = 0;
  for(int j=0; j<n; j++) {
    sum += A[j] * A[j];
...
void nrm2(double *restrict out, const double *restrict in, int n)
{
  double mg = mag(in, n);
  for(int i=0; i<n; i++) {
    out[i] = in[i] / mg;
...
void nrm3(double *restrict out, const double *restrict in, int n)
{
  for(int i=0; i<n; i++) {
    out[i] = in[i] / sqrt(mg2(in, n));
  }
}
