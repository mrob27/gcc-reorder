...
{
fprintf(rml_dbf1, "backprop:: instantiate\n");
  bitmap_clear (m_visited_blocks);
}

backprop::~backprop ()
{
fprintf(rml_dbf1, "~backprop\n");
...
backprop::execute ()
{
fprintf(rml_dbf1, "backprop::execute() begin\n");
  ...
  if (dump_file && (dump_flags & TDF_DETAILS))
    fprintf (dump_file, "\n");
fprintf(rml_dbf1, "backprop::execute() done\n");
}
...
{
fprintf(rml_dbf1, "pass_backprop::execute C\n");
  backprop (fn).execute ();
  return 0;
}
