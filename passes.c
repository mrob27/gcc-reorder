...
/* %%% RML1
   dbf1.h: To write to debug log file
   ord1.h: To know the requested pass ordering */
#include "dbf1.h"
#include "ord1.h"
...
/* fprintf(rml_dbf1, "registering pass '%s'\n", full_name); */
...
{
  opt_pass **p;
  fprintf(rml_dbf1, "pm::get_pass_by_name(%s)\n", name);
  p = m_name_to_pass_map->get (name);
  if (p) {
    return *p;
  }
...
{
  opt_pass **p;
  fprintf(rml_dbf1, "pass_manager::pass_manager start\n");
...
#include "ins2.c"
  ...
  fprintf(rml_dbf1, "pass_manager::pass_manager finished\n");
}
...
/* %%% RML1
  execute.pass_list_1 (in newer GCC versions) is the same as execute_pass_list
  (in older GCC versions), and contains a simple loop that walks through a
  linked list of passes, but can also call itself recursively when a non-NULL
  .sub attribute is found. Here it is expanded from the original 10 lines
  of code, to add the ability to call the passes in a different order (by
  repeatedly traversing the list and deciding which one to call on each
  traversal). Much of the new code is just for logging
    The original execute.pass_list_1 took only the first argument
  "opt_pass *pass" and contained only the following code:

    p = pass;
    do {
      gcc_assert (p->type == GIMPLE_PASS || p->type == RTL_PASS);
      if (cfun == NULL) { return; }
      gcc_assert(p->name);
      if (execute_one_pass (p) && p->sub) {
        execute.pass_list_1 (p->sub);
      }
      p = p->next;
    } while (p);

*/
static void execute_pass_list_1 (opt_pass *pass, opt_pass *parent, int depth)
{
  int i, j, n_passes, n1, n2, n_subs;
  int reorder_step, active, n_reorders, group_num, do_this;
  opt_pass *p;
  const char * do_now_name;
  int do_now_repeat;
  int do_repeat;

  active = 0;
  n_reorders = 0;
  for(j=0;j<depth;j++){fprintf(rml_dbf1," .");}
  if (parent) {
    fprintf(rml_dbf1, "epl1(%s)", parent->name);
  } else {
    fprintf(rml_dbf1, "epl1()");
  }
  if (parent) {
    /* We are not the root node, check to see if our children have a
       custom reorder list */
    if (ord1_group_match(parent->name, &group_num, &n_reorders)) {
      fprintf(rml_dbf1, " is target (group %d)", group_num);
      active = 1;
      fprintf(rml_dbf1, ", n_reorders = %d", n_reorders);
    }
  } else {
    fprintf(rml_dbf1, " is root group");
  }

  /* Count the number of passes in this list, and note any that have subs.
     We also use this as an opportunity to generate the report of default
     passes in each group (as groupwise lists rather than depth-first
     tree traversal order). */
  if (parent) {
    fprintf(rml_rpt2, "\n%s:\n", parent->name);
  }
  for(n_passes=0, p=pass; p; n_passes++) {
    /* if (parent) print p->name to rpt2 */
    if (parent && p && (p->name)) {
      fprintf(rml_rpt2, "  %s%s\n", p->name, ((p->sub) ? "  <" : ""));
    }
    p = p->next;
  }
  fprintf(rml_dbf1, ", n_passes=%d", n_passes);

  /* Count the number of subs */
  for(n_subs=0, p=pass; p; ) {
    if (p->sub) {
      n_subs++;
    }
    p = p->next;
  ...
  /* If there are subs, list their indices */
  if (n_subs) {
    fprintf(rml_dbf1, " (%d subs: ", n_subs);
    for (n1=n2=0, p=pass; p; n2++) {
      if (p->sub) {
        if (n1) { fprintf(rml_dbf1, ", "); }
        fprintf(rml_dbf1, "%d", n2);
        n1++;
      }
      p = p->next;
    }
  ...
  /* Repeat once for each item in the reorder directive, plus a final pass to
     do everything that remains. */
  for(reorder_step=0; reorder_step<=n_reorders; reorder_step++) {
    fprintf(rml_dbf1, "reorder_step %d", reorder_step);
    /* Find the name of the Nth item in the reorder directive */
    if (active) {
      do_now_name = ord1_index(group_num, reorder_step, &do_now_repeat);
      fprintf(rml_dbf1, ", do_now_name=='%s'", do_now_name);
    } else {
      do_now_name = 0;
    }
    fprintf(rml_dbf1, "\n");

    /* Inner loop: For each pass in the default ordering: */
    p = pass; i = 0;
    do {
      gcc_assert (p->type == GIMPLE_PASS || p->type == RTL_PASS);
      if (cfun == NULL) { return; }
      ...
      /* Decide whether we are going to execute this pass on this particular
         reorder step */
      do_this = 0; do_repeat = 0;
      if (reorder_step < n_reorders) {
        /* If we are at a step less than N, then:
           If this pass' name is equal to the Nth name in the reorder
           directive, then execute it; otherwise skip        */
        do_this = (do_now_name && (rpm_strcmp(p->name, do_now_name) == 0));
        if (do_this) {
          do_repeat = do_now_repeat;
        }
      } else {
        int tr;
        /* otherwise (step N):
           If the reorder directive list is empty OR if this pass' name does
           not appear in the reorder directive, then execute this pass now. */
        if (n_reorders == 0) {
          do_this = 1;
          do_repeat = 1;
        } else if (ord1_lookup(group_num, p->name, &tr) < 0) {
          do_this = 1;
          do_repeat = tr;
        }
      }
      if (do_this) {
        for(j=0; j<depth; j++) { fprintf(rml_dbf1, " ."); }
        fprintf(rml_dbf1, " vv *do_this*\n");
      }
      for(j=0; j<depth; j++) { fprintf(rml_dbf1, " ."); }
      fprintf(rml_dbf1, " %2d.%s\n", i, p->name);

      if (do_this && (do_repeat >= 0) && (do_repeat < 10)) {
        /* okay */
      } else if ((do_this == 0) && (do_repeat == 0)) {
        /* okay */
      } else {
        fprintf(rml_dbf1, "\n\nERROR: Cannot handle do_this == %d and do_repeat == %d\n",
                                                     do_this, do_repeat);
        fflush(rml_dbf1);
        dbf1_fatal_exit(-103);
      }

      // if (reorder_step == 0) { /* Use this to disable reordering */
      if (do_this && (do_repeat > 0)) {
        bool eop_result;
        int k;
        for(j=0; j<depth; j++) { fprintf(rml_dbf1, " ."); }
        fprintf(rml_dbf1, ":_: %s x %d\n", p->name, do_repeat);

        for(j=0; j<depth; j++) { fprintf(rml_dbf1, " ."); }
        fprintf(rml_dbf1, " %2d>%s x%d ", i, p->name, do_repeat);
        for(k=0; k<do_repeat; k++) {
          if (k>0) {
            for(j=0; j<depth; j++) { fprintf(rml_dbf1, " ."); }
            fprintf(rml_dbf1, "  (repetition %d)\n", k);
          }
          fflush(rml_dbf1);
          eop_result = execute_one_pass (p);
          fprintf(rml_dbf1, " -> %c\n", eop_result ? 'T' : '0');
          if (eop_result && p->sub) {
            execute_pass_list_1 (p->sub, p, depth+1);
          }
        }
      }
  ...
  for(j=0;j<depth;j++){fprintf(rml_dbf1,"  ");} fprintf(rml_dbf1, "epl1 done\n");
} /* End of execute.pass_list_1 */

void
execute_pass_list (function *fn, opt_pass *pass)
{
  fprintf(rml_dbf1, "epl(%16lx : %s) spn=%d\n",
               ((unsigned long) pass), pass->name, pass->static_pass_number);
  gcc_assert (fn == cfun);
  execute_pass_list_1 (pass, 0, 1);
  if (cfun && fn->cfg)
    {
      free_dominance_info (CDI_DOMINATORS);
      free_dominance_info (CDI_POST_DOMINATORS);
