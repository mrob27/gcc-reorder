...
/* %%% RML1
   GCC (since versions 3.0x) by design reads environment variables in only one
   place, the process_command function here in gcc.c)

   getenv is unpoisoned so that RML1_ORD_PATH can be read (and immediately
   used only once, and not changed) by ord1.c, which is included by both main.c
   and gcc-main.c, as gcc.c is not linked into the build at every stage of the
   build process. The original pragma was:
